...
import { getDashboard, Dashboard, addWidget, removeWidget } from "../api/dashboards";
...
const DashboardView: React.FC = () => {
  ...
  const handleAddWidget = (type: string, title: string) => {
    if (!dashboard) return;
    addWidget(dashboard.id, { widget_type: type, title }).then(() => {
  ...
  const handleRemoveWidget = (widgetId: number) => {
    if (!dashboard) return;
    removeWidget(dashboard.id, widgetId).then(() => load());
  };
  ...
  if (!dashboard) {
    return <div className="empty-state"><div className="empty-state-title">Dashboard not found</div></div>;
  }

  const activeWidgetTypes = dashboard.widgets.map(w => w.widget_type);

  const renderWidget = (type: string, widgetId: number, title: string) => {
    switch (type) {
      ...
      default:
        return <div style={{ padding: 24, color: "var(--color-text-muted)", textAlign: "center" }}>Unknown widget type: {type}</div>;
    }
  };
    ...
    <div>
      ...
      <div className="dv-toolbar">
        <div className="dv-breadcrumb">
          <Link to="/dashboards" className="dv-breadcrumb-link">All Dashboards</Link>
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polyline points="9 18 15 12 9 6" /></svg>
          <span className="dv-breadcrumb-current">{dashboard.name}</span>
        </div>
          ...
          {dashboard.widgets.map(w => (
            <div key={w.id} className={`dv-widget ${w.widget_type === "LAUNCH_TABLE" || w.widget_type === "MOST_FAILED" ? "dv-widget-full" : ""}`}>
              ...
              <div className="dv-widget-body">
                {renderWidget(w.widget_type, w.id, w.title)}
              </div>
            </div>
          ))}
        </div>
